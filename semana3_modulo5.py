...
conexao = sqlite3.connect('banco.html')
cursor = conexao.cursor()
sql = 'insert into categoria (titulo, data, descricao) values (?, ?, ?)'

navegador = Chrome(service=Service(ChromeDriverManager().install()))
navegador.get('http://raspagem.herokuapp.com/noticias/')
link = navegador.find_element(By.LINK_TEXT, 'Tempo Real')
link.click()
...
for noticia in navegador.find_element(By.css_SELECTOR, 'div.position-relative'):
    ...
    data = noticia.find_element(By.CSS_SELECTOR, 'div.text_muted').text
